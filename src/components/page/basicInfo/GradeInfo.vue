<template>
	<div class="container">
		<div class="handle-box">
			...
			<div class="demo-input-suffix">
				年级名称：
				<el-input v-model="gradeName" placeholder="" clearable class="handle-input-md m-r-10"></el-input>
			</div>
			<div class="demo-input-suffix">
				数据状态：
        ...
        <el-table :data="data" border class="table" stripe ref="multipleTable" >
            <el-table-column prop="id" label="年级id" align="center" width="260">
            </el-table-column>
	...
	export default{
        ...
        mounted() {
         	...
         	let curRights = rights.filter(function(item){
         		return item.rightId.split(":")[0] == 'grade';
         	})
         	let that = this;
        ...
        methods: {
            ...
            // 获取 list数据
            getData() {
                this.$axios.get("app/study/grade/list",{
                    params:{
		    			"periodId": this.periodId, // 学段
		    			"dataStatus": this.dataStatus, // 数据状态
		    			"gradeName": this.gradeName, // 年级名称
		    			"rows": 10, // 每页记录数，默认为25
            ...
            getDetails() { // 获取详情
            	this.$axios.get("app/study/grade/get",{
                    params:{
		    			"id": this.form.id // ID
		    		}
                }).then((res) => {
                	if(res.status == 200 && res.data.code == '0000'){
	                	...
	                	this.form.gradeName = res.data.data.gradeName;
            ...
            sure() { // 新增编辑确定
            	let _url = '';
            	if(this.form.id){
            		_url = "app/study/grade/update"; // 编辑
            	}else{
            		_url = "app/study/grade/add"; // 新增
            	}
            ...
            sureDel() { // 确定删除
            	this.$axios.get("app/study/grade/disable",{
                    params:{
            ...
            sureEnable() { // 确定恢复
            	this.$axios.get("app/study/grade/enable",{
                    params:{
